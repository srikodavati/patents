...
def process_xml_files_in_directory(directory_path, output_csv_file):
    all_extracted_data = []
    file_count = len([name for name in os.listdir(directory_path) if name.endswith('.xml')])  # Count XML files
    processed_count = 0
    print(file_count)  # Initialize processed file counter
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.xml'):
            ...
            processed_count += 1  # Increment the processed file counter
            print(f"Processed {filename}. ({processed_count}/{file_count} files done)")
...
directory_path = '/Users/srilekhakodavati/Desktop/Patents/2020/2'
output_csv_file = '/Users/srilekhakodavati/Desktop/Patents/files/20_2.csv'
